...
- **Parallelism is non-negotiable across MCP lanes in step 2 of `orchestrate-multi-ai`.** Tavily (`research-scout`) normally runs as a background Agent and must overlap wall time with the browser lane. Within the browser lane, `browser-operator` calls are serialized because `chrome-devtools` MCP is a single stateful connection to one Chrome process - parallel calls clobber each other's selected page. If the background Tavily lane cannot produce `runs/<slug>/raw/tavily/status.json` with `completed=true`, fall back to inline Tavily in the same run. Never sequentialize the whole fan-out; never parallelize two `browser-operator` calls.
